...
SCS::SCS()
{
	Level = 1;//All non-broadcast instructions return acknowledgment
	Error = 0;
}
...
//Servo ID, MemAddr memory address, write data, write length
...
//Asynchronous write execute instruction
//Servo ID
...
//Servo ID[] array, IDN array length, MemAddr memory address, write data, write length
...
//Read instruction
//Servo ID, MemAddr memory address, return data nData, data length nLen
...
//Ping instruction, returns servo ID, -1 on timeout
int	SCS::Ping(u8 ID)
{
	rFlushSCS();

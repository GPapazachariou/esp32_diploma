...
public:
	...
	virtual int WritePos(u8 ID, u16 Position, u16 Time, u16 Speed);//Standard write single servo position command
	virtual int WritePosEx(u8 ID, s16 Position, u16 Speed, u8 ACC);//Single servo position command
	virtual int RegWritePos(u8 ID, u16 Position, u16 Time, u16 Speed = 0);//Asynchronous write single servo position command (takes effect with RegWriteAction)
	virtual void SyncWritePos(u8 ID[], u8 IDN, u16 Position[], u16 Time[], u16 Speed[]);//Synchronous write multiple servo position command

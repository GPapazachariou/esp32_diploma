...
public:
	...
	virtual int WritePosEx(u8 ID, s16 Position, u16 Speed, u8 ACC = 0);//Standard write single servo position command
	virtual int RegWritePosEx(u8 ID, s16 Position, u16 Speed, u8 ACC = 0);//Asynchronous write single servo position command (takes effect with RegWriteAction)
	virtual void SyncWritePosEx(u8 ID[], u8 IDN, s16 Position[], u16 Speed[], u8 ACC[]);//Synchronous write multiple servo position command
	...
	virtual int CalibrationOfs(u8 ID);//Center calibration
	virtual int FeedBack(int ID);//Read back all servo feedback data
	virtual int ReadPos(int ID);//Read position
	virtual int ReadSpeed(int ID);//Read speed
	virtual int ReadLoad(int ID);//Read motor output voltage percentage (0~1000)
